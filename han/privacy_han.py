...
def train(train_loader):
    ...
    save_folder_path = f"/home/zhuohang/SGG-GCN/pth/hanpth/hanpth_normal/{training_time}/"
    if not os.path.exists(save_folder_path):
        os.makedirs(save_folder_path)
    # model_path = './hanpth/'
    metadata = (['object', 'relation'], [('object', 'to', 'relation'), ('relation', 'to', 'object')])
    num_classes = 2
    # model = HAN(-1, 64, 32 ,num_classes, metadata=metadata).to(device)
    ...
    for epoch in tqdm(range(100)):
        for graph in tqdm(train_loader):
            ...
            f = model(graph)
            loss = loss_function(f[train_mask].squeeze(), y[train_mask])
            optimizer.zero_grad()
...
def test(model, mask):
    ...
    with torch.no_grad():
        ...
        for i,graph in enumerate(train_loader):
            ...
            out = model(graph)
            loss_function = torch.nn.CrossEntropyLoss().to(device)
            loss = loss_function(out[mask].squeeze(), y[mask])
            _, pred = out.max(dim=1)
            if(i % 100 == 0):
                print(f"true:{y}")
                print(f"pred:{pred}")   
            correct = correct + int(pred[mask].eq(y[mask]).sum().item())
            # test_num += test_num + int(test_mask.sum())
            test_num += len(mask)
            # print(test_num)
        # print(f"correct : {correct}")
        # print(f"mask_len:{test_num}")
