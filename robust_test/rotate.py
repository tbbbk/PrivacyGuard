# rotate equals to some relations are wrong
import sys
import yaml
...
rotate_mosaic_pred_name = "rotate_random_balance_custom_prediction.json"
rotate_privacy_pred_name = "rotate_random_balance_custom_prediction.json"
...
category_num = len(mosaic_info["ind_to_predicates"])-1
num_rate = 2
# 1/10 is wrong
for i in tqdm(range(len(mosaic_pred))):
    node_categroy = mosaic_pred[str(i)]["rel_labels"]
    for j in range(len(node_categroy)):
        if(rand.randint(0,num_rate) != 0):
            node_categroy[j] = rand.randint(0,category_num)
    mosaic_pred[str(i)]["rel_labels"] = node_categroy

with open(f"{mosaic_save_dir}/{rotate_mosaic_pred_name}", "w") as json_file:
    json.dump(mosaic_pred, json_file)



for i in tqdm(range(len(privacy_pred))):
    node_categroy = privacy_pred[str(i)]["rel_labels"]
    for j in range(len(node_categroy)):
        if(rand.randint(0,num_rate) == 0):
            node_categroy[j] = rand.randint(0,category_num)
    privacy_pred[str(i)]["rel_labels"] = node_categroy

with open(f"{privacy_save_dir}/{rotate_privacy_pred_name}", "w") as json_file:
